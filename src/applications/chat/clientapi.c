                       ...
                       GNUNET_CHAT_MessageCallback callback, void *cls)
{
  ...
  size_of_join = sizeof (CS_chat_JOIN_MESSAGE) + strlen(nickname) + sizeof(GNUNET_RSA_PublicKey) + strlen(room_name);
  ...
  memcpy(&join_msg->nick[strlen(nickname) + sizeof(GNUNET_RSA_PublicKey)],room_name,strlen(room_name));
  
  join_msg->header = csHdr;
  join_msg->header.size = htons(size_of_join);
  
  if (GNUNET_SYSERR == GNUNET_client_connection_write (sock, &join_msg->header))
  {
                          ...
                          const GNUNET_RSA_PublicKey * receiver)
{
  ...
  cs_msg_hdr.size = htons (sizeof (GNUNET_MessageHeader) + sizeof(CS_chat_MESSAGE) + strlen(room->nickname) + strlen(message) + strlen(room->room_name));
  ...
  memcpy(&msg_to_send->nick[strlen(room->nickname) + strlen(message)],room->room_name,strlen(room->room_name));
  ...
  if (GNUNET_SYSERR == GNUNET_client_connection_write (room->sock,&msg_to_send->header))
  {
  	fprintf (stderr, _("Error writing to socket.\n"));
    ret = GNUNET_SYSERR;
  }
