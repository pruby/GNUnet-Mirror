...
/* These selects basically work in the same way as the old MySQL selects do. But because the
   reverse ordering kills performance, we are using subselects with MAX() for SQLite instead.

#define SELECT_IT_NON_ANONYMOUS "SELECT size, type, prio, anonLevel, expire, GNUNET_hash, value, _ROWID_ FROM gn070 WHERE ( (prio = ? AND GNUNET_hash < ?) OR (prio < ? AND GNUNET_hash != ?) ) "\
                                "AND anonLevel=0 AND type not in (4294967294, 4294967295) "\
                                "ORDER BY prio DESC,GNUNET_hash DESC LIMIT 1"

#define SELECT_IT_MIGRATION_ORDER "SELECT size, type, prio, anonLevel, expire, GNUNET_hash, value, _ROWID_ FROM gn070 WHERE ( (expire = ? AND GNUNET_hash < ?) OR (expire < ? AND GNUNET_hash != ?) ) "\
                                  "AND expire > ? AND type!=3 "\
                                  "ORDER BY expire DESC,GNUNET_hash DESC LIMIT 1"
*/

#define SELECT_IT_LOW_PRIORITY "SELECT size, type, prio, anonLevel, expire, hash, value, _ROWID_ FROM gn070 WHERE " \
  "((prio = ? AND hash > ?) OR (prio > ? AND hash != ?)) AND type not in (4294967294, 4294967295, 3) ORDER BY " \
  "prio ASC,hash ASC LIMIT 1"

#define SELECT_IT_NON_ANONYMOUS  "SELECT size, type, prio, anonLevel, expire, hash, value, _ROWID_ FROM gn070 " \
  "WHERE hash = " \
    "(SELECT MAX(hash) FROM gn070 WHERE prio = " \
      "(SELECT MAX(prio) FROM gn070 WHERE ((prio = ? AND hash < ?) OR (prio < ? AND hash != ?)) AND anonLevel=0 AND " \
        "type NOT IN (4294967294, 4294967295, 3)) " \
      "AND ((prio = ? AND hash < ?) OR (prio < ? AND hash != ?)) AND anonLevel=0 AND type NOT IN (4294967294, " \
      "4294967295, 3))"

#define SELECT_IT_EXPIRATION_TIME "SELECT size, type, prio, anonLevel, expire, hash, value, _ROWID_ FROM gn070 WHERE " \
  "((expire = ? AND hash > ?) OR (expire > ? AND hash != ?)) AND type NOT IN (4294967294, 4294967295, 3) ORDER BY " \
  "expire ASC,hash ASC LIMIT 1"

#define SELECT_IT_MIGRATION_ORDER "Select size, type, prio, anonLevel, expire, hash, value, _ROWID_ FROM gn070 WHERE " \
  "hash =" \
    "(Select MAX(hash) FROM gn070 WHERE expire =" \
        "(Select MAX(expire) FROM gn070 WHERE ((expire = ? AND hash < ?) OR (expire < ? AND hash != ?)) AND " \
        "type NOT IN (4294967294, 4294967295, 3)) " \
    "and type not in (4294967294, 4294967295, 3) and ((expire = ? and hash < ?) or (expire < ? and hash != ?)))"
...
createIndices (sqlite3 * dbh)
{
  ...
  CHECK (SQLITE_OK ==
         sqlite3_exec (dbh,
                       "CREATE INDEX idx_comb4 ON gn070 (anonLevel,type,prio,hash)",
                       NULL, NULL, ENULL));
  CHECK (SQLITE_OK ==
         sqlite3_exec (dbh,
...
getDBHandle ()
{
  ...
  if ((sq_prepare (ret->dbh,
    ...
    {
      LOG_SQLITE (ret,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "precompiling");
...
delete_by_rowid (sqliteHandle * handle, unsigned long long rid)
{
  ...
  if (sq_prepare (handle->dbh,
    ...
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sq_prepare");
  ...
  if (SQLITE_DONE != sqlite3_step (stmt))
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite3_step");
      sqlite3_finalize (stmt);
      return GNUNET_SYSERR;
    }
  sqlite3_finalize (stmt);
  ...
  if (contentSize < 0)
    {
      ...
      if (SQLITE_OK != sqlite3_reset (stmt))
        LOG_SQLITE (handle,
                    GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite3_reset");
      if (sq_prepare (dbh,
...
getStat (sqliteHandle * handle, const char *key)
{
  ...
  if (i != SQLITE_OK)
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite_getStat");
      return GNUNET_SYSERR;
    }
  return ret;
}
...
setStat (sqliteHandle * handle, const char *key, unsigned long long val)
{
  ...
  if ((SQLITE_OK !=
      ...
      || (SQLITE_OK != sqlite3_bind_int (stmt, 3, GNUNET_GNUNET_ECRS_BLOCKTYPE_RESERVED)))
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK,
                  "sqlite3_bind_xxx");
...
sqlite_iterate (unsigned int type,
                ...
                unsigned int iter_select, GNUNET_DatastoreValueIterator iter, void *closure)
{
  sqlite3_stmt *stmt;
  int count;
  char *stmt_str;
  ...
  if (sq_prepare (dbh, stmt_str, &stmt) != SQLITE_OK)
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite3_prepare");
      GNUNET_mutex_unlock (lock);
      return GNUNET_SYSERR;
    }
  count = 0;
 ...
 */
static int
iterateLowPriority (unsigned int type, GNUNET_DatastoreValueIterator iter, void *closure)
{
 ...
 */
static int
iterateNonAnonymous (unsigned int type, GNUNET_DatastoreValueIterator iter, void *closure)
{
 ...
 */
static int
iterateExpirationTime (unsigned int type, GNUNET_DatastoreValueIterator iter, void *closure)
{
  ...
  if (count != GNUNET_SYSERR)
    {
      /* re-computed payload! */
      GNUNET_GE_LOG (ectx,
              GNUNET_GE_INFO | GNUNET_GE_IMMEDIATE | GNUNET_GE_USER | GNUNET_GE_ADMIN,
...
#if DEBUG_SQLITE
  GNUNET_GE_LOG (ectx,
          GNUNET_GE_DEBUG | GNUNET_GE_REQUEST | GNUNET_GE_USER, "SQLite: closing database\n");
#endif
  ...
  for (idx = 0; idx < handle_count; idx++)
    {
      ...
      if (sqlite3_close (h->dbh) != SQLITE_OK)
        LOG_SQLITE (h,
                    GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite_close");
      GNUNET_free (h);
    }
  GNUNET_free (handles);
  handles = NULL;
     ...
     unsigned int type, GNUNET_DatastoreValueIterator iter, void *closure)
{
  ...
  if (sq_prepare (dbh, scratch, &stmt) != SQLITE_OK)
    {
      LOG_SQLITE (handle,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite_prepare");
      GNUNET_mutex_unlock (lock);
      return GNUNET_SYSERR;
    }
  count = 0;
...
#if DEBUG_SQLITE
  GNUNET_GE_LOG (ectx,
          GNUNET_GE_DEBUG | GNUNET_GE_REQUEST | GNUNET_GE_USER, "SQLite: done writing content\n");
#endif
  GNUNET_mutex_unlock (lock);
  return GNUNET_OK;
}
...
update (unsigned long long uid, int delta, GNUNET_CronTime expire)
{
  ...
  if (n != SQLITE_DONE)
    LOG_SQLITE (dbh, GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite3_step");

  sqlite3_reset (dbh->updPrio);

#if DEBUG_SQLITE
  GNUNET_GE_LOG (ectx, GNUNET_GE_DEBUG | GNUNET_GE_REQUEST | GNUNET_GE_USER, "SQLite: block updated\n");
#endif
  GNUNET_mutex_unlock (lock);
  if (n == SQLITE_BUSY)
    return GNUNET_NO;
...
#if DEBUG_SQLITE
  GNUNET_GE_LOG (ectx,
          GNUNET_GE_DEBUG | GNUNET_GE_REQUEST | GNUNET_GE_USER, "SQLite: initializing database\n");
#endif
  ...
  if (payload == GNUNET_SYSERR)
    {
      GNUNET_GE_BREAK (ectx, 0);
      LOG_SQLITE (dbh,
                  GNUNET_GE_ERROR | GNUNET_GE_ADMIN | GNUNET_GE_USER | GNUNET_GE_BULK, "sqlite_payload");
...
#if DEBUG_SQLITE
  GNUNET_GE_LOG (ectx,
          GNUNET_GE_DEBUG | GNUNET_GE_REQUEST | GNUNET_GE_USER, "SQLite: database shutdown\n");
#endif
  GNUNET_mutex_destroy (lock);
  lock = NULL;
  coreAPI = NULL;
