...
void
swapNodes(struct GNUNET_dv_heap_node *first, struct GNUNET_dv_heap_node *second, struct GNUNET_dv_heap *root)
{
...
percolateHeap(struct GNUNET_dv_heap_node *pos, struct GNUNET_dv_heap *root)
{
	...
	while((pos->parent != NULL) &&
			(((root->type == GNUNET_DV_MAX_HEAP) && (pos->parent->neighbor->cost < pos->neighbor->cost)) ||
			((root->type == GNUNET_DV_MIN_HEAP) && (pos->parent->neighbor->cost > pos->neighbor->cost))))
	{
...
percolateDownHeap(struct GNUNET_dv_heap_node *pos, struct GNUNET_dv_heap *root)
{
	...
	if ((root->type ==  GNUNET_DV_MAX_HEAP))
	{
		if ((pos->left_child != NULL) && (pos->left_child->neighbor->cost > switchNeighbor->neighbor->cost))
		{
			switchNeighbor = pos->left_child;
		}

		if ((pos->right_child != NULL) && (pos->right_child->neighbor->cost > switchNeighbor->neighbor->cost))
		{
...
GNUNET_DV_Heap_updatedCost(struct GNUNET_dv_heap *root, struct GNUNET_dv_neighbor *neighbor)
{
	...
	if ((root->type == GNUNET_DV_MAX_HEAP) && (parent != NULL) && (node->neighbor->cost > parent->neighbor->cost))
		percolateHeap(neighbor->max_loc, root);
	else if ((root->type == GNUNET_DV_MIN_HEAP) && (parent != NULL) && (node->neighbor->cost < parent->neighbor->cost))
...
int
GNUNET_DV_Heap_delete_matching_referrers(struct GNUNET_dv_heap *root, struct GNUNET_dv_heap_node *node, GNUNET_PeerIdentity *toMatch)
{
	if (node->left_child != NULL)
	{
		GNUNET_DV_Heap_delete_matching_referrers(root, node->left_child, toMatch);
	}
	if (node->right_child != NULL)
	{
		GNUNET_DV_Heap_delete_matching_referrers(root, node->right_child, toMatch);
	}
	if ((node->neighbor != NULL) && (memcmp(node->neighbor, toMatch, sizeof(struct GNUNET_PeerIdentity)) == 0))
	{
